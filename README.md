...
**注意：此仓库暂未包含所有 cookie 同步方法，您可以提交 issue 请求适配。若您愿意贡献新的同步方法，请参照 [贡献](#贡献) 添加新的 JS 文件并创建 PR。**
...
2. 将本仓库根目录映射为 RSSHub 镜像中的 `/app/cookiecloud`。
3. 添加环境变量：
...
4. 修改启动命令为 `node /app/cookiecloud/index.js`。
5. 日志中出现了 `CookieCloud loaded.` 即为加载成功。
...
当您希望贡献一个网站的同步方法，您需要在 [cookiecloud/cookies](/libs/cookies) 中添加 JS 文件。
...
+ 每个 JS 文件导出一个 object，key 是 RSSHub 所需的环境变量名称，value 是一个 array。
